...
export const blogPostBySlugQuery = `*[_type == "blogPost" && slug.current == $slug && defined(publishedAt) && (!defined(migrationStatus) || migrationStatus in ["approved", "published"])][0]`;
export const blogPostSlugsQuery = `*[_type == "blogPost" && defined(slug.current) && defined(publishedAt) && (!defined(migrationStatus) || migrationStatus in ["approved", "published"])][].slug.current`;
